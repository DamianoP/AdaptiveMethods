...
def printFigure(data,classifierNames,title,precision):
  ...
  plt.title("Accuracy\n "+title+" with precision "+precision+"\nA=accuracy, S=performance increase",y=0.85)
...
def mainFunction(precision,networkNames,classifierNames,title,crossValidation):
  value=[]
  for i in range(0,len(networkNames)):
    localValue=[]
    localValue.append(networkNames[i])
    for j in range(1,len(classifierNames)):
      folderName=networkNames[i]+"_"+classifierNames[j]+"_"+crossValidation
...
mainFunction("default",networkNames,classifierNames,title,"ALL")
mainFunction("uint8",networkNames,classifierNames,title,"ALL")

title="firefly_CV"#raw_input("Image title: ")
mainFunction("default",networkNames,classifierNames,title,"CV")
mainFunction("uint8",networkNames,classifierNames,title,"CV")
